...
#           hitsWall = self.walls[nextx][nexty]
        successors = []
        cornersTuple = state[1]
        cornersList = list(cornersTuple)
        if (state[0] in self.corners):  #updates state of corners
            count = 0
            for corner in self.corners:
                if (state[0] == corner):
                    cornersList[count] = True
                count = count+1
        cornersTuple = tuple(cornersList)
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
                x, y = state[0]
                dx, dy = Actions.directionToVector(action)
                nextx, nexty = int(x + dx), int(y + dy)
                hitsWall = self.walls[nextx][nexty]
                if (not hitsWall):
                    successors.append((((nextx, nexty), cornersTuple), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for i, corner in enumerate(problem.corners):
        if(not state[1][i]):
            cornerdist = ((abs(state[0][0] - corner[0]) + abs(state[0][1] - corner[1])))
...
def foodHeuristic(state, problem):
    ...
    if(foodList.__len__()==0):
     return 0
    for food in foodList:
     c=c+1
     s.append(util.manhattanDistance(position,food))
...
#     return search.astar(problem,foodHeuristic(curstate,problem))[0]
#     return search.astar(problem,manhattanHeuristic(pacpoint,problem))[0]
#,foodHeuristic(curstate,problem))
